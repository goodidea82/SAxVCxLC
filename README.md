...
* Additions, modification, HD-content, scripts, etc for SAxVCxLC are very welcome and should be provided as external mods for three reasons: (1) to ensure that SAxVCxLC becomes a stable basis for modding, (2) to not force modification/addition on users, but let them customize it individually, (3) to advertise SAxVCxLC on other websites. Many GTA-specialized websites exists where you can publish mods. In your mod provide a Readme.txt with a link to a release version on this GitHub. Here we can provide a list of mods that link to SAxVCxLC on GitHub.
